...
Write-Host "2. Preview Splash UI (Updater/Splash)"
    ...
    Write-Host "4. Preview Message UI (Error/Alert)"
    Write-Host "5. Build All Bins (Verify compilation)"
    Write-Host "Q. Quit"
    ...
    if ($choice -eq "1") {
        ...
        Write-Host "Starting Message UI Preview..." -ForegroundColor Green
        cargo test -p velopack_bins windows::splash::tests::test_show_message -- --ignored --nocapture
    } elseif ($choice -eq "5") {
